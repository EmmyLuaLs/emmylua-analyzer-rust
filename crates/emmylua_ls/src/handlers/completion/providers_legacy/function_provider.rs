...
fn get_token_should_type(builder: &mut CompletionBuilder) -> Option<Vec<LuaType>> {
    ...
    // Allow walking upward when an empty string literal `""` is typed.
    if LuaLiteralExpr::can_cast(parent_node.kind().into()) {
...
fn add_union_member_completion(
    ...
    // If there is a StrTplRef, move it to the end.
...
fn infer_param_list(
    ...
    for doc_param in doc_params {
        ...
        if !names.contains(&name) {
            // Do not add a completion here; the joined form should take precedence over individual ones.
    ...
    // Remove names that already exist in params.
    ...
    // Do not return types: string types would be quoted, but unquoted strings are needed here, so add them directly.
    None
}
...
fn infer_call_arg_list(
    ...
    for call_expr_func in call_expr_funcs {
        ...
        {
            // Convert to a form that is easier to compare.
...
fn add_enum_members_completion(
    ...
    // Determine whether this is triggered by a string literal.
...
/// Ensures all members are function or nil, then returns the union type of the functions, or None if any member is not a function.
