...
fn complete_provider(builder: &mut CompletionBuilder) -> Option<()> {
    ...
    match parent_node {
        ...
        // Completions triggered inside a string.
...
fn supports_provider(builder: &CompletionBuilder) -> bool {
    ...
    if builder.trigger_kind == CompletionTriggerKind::TRIGGER_CHARACTER {
        ...
        // Even when explicitly invoked, do not allow completion in function parameter lists.
...
pub fn add_global_env(
    ...
    for decl_id in global_env.iter() {
        ...
        // If the range is the same, this is defining a new global variable, so do not add it.
...
fn env_check_match_word(trigger_text: &str, name: &str) -> bool {
    // If the first character is `(` or `,`, allow it so completion can trigger in function argument calls.
    if matches!(trigger_text.chars().next(), Some('(') | Some(',')) {
        return true;
    }

    if check_match_word(trigger_text, name) {
        // If the first character matches, check that every character in trigger_text exists in name.
