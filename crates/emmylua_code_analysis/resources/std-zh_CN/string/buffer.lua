...
--- @param size integer
--- @return ffi.cdata* ptr 指向该空间的 `uint8_t *` FFI cdata 指针。
--- @return integer len    可用长度（字节）。
...
--- @return ffi.cdata* ptr 指向缓冲区数据的 `uint8_t *` FFI cdata 指针。
--- @return integer len    缓冲区数据的字节长度。
