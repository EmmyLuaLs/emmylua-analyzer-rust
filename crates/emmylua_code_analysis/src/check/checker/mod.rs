...
pub fn check_file(semantic_model: &SemanticModel<'_>, config: Arc<CheckConfig>) -> Vec<Diagnostic> {
    ...
    run_checked::<unused::UnusedChecker>(&mut context, semantic_model);
    run_checked::<deprecated::DeprecatedChecker>(&mut context, semantic_model);
    run_checked::<undefined_global::UndefinedGlobal>(&mut context, semantic_model);
    run_checked::<undefined_doc_param::UndefinedDocParamChecker>(&mut context, semantic_model);
    run_checked::<unbalanced_assignments::UnbalancedAssignmentsChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<redefined_local::RedefinedLocalChecker>(&mut context, semantic_model);
    run_checked::<duplicate_require::DuplicateRequireChecker>(&mut context, semantic_model);
    run_checked::<duplicate_type::DuplicateTypeChecker>(&mut context, semantic_model);
    run_checked::<type_access_modifier::InconsistentTypeAccessModifierChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<circle_doc_class::CircleDocClassChecker>(&mut context, semantic_model);
    run_checked::<unknown_doc_tag::UnknownDocTagChecker>(&mut context, semantic_model);
    run_checked::<local_const_reassign::LocalConstReassignChecker>(&mut context, semantic_model);
    run_checked::<param_type_check::ParamTypeChecker>(&mut context, semantic_model);
    run_checked::<return_type_mismatch::ReturnTypeMismatchChecker>(&mut context, semantic_model);
    run_checked::<assign_type_mismatch::AssignTypeMismatchChecker>(&mut context, semantic_model);
    run_checked::<code_style::invert_if::InvertIfChecker>(&mut context, semantic_model);
    run_checked::<
        code_style::non_literal_expressions_in_assert::NonLiteralExpressionsInAssertChecker,
    >(&mut context, semantic_model);
    run_checked::<code_style::preferred_local_alias::PreferredLocalAliasChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<generic_constraint_mismatch::GenericConstraintMismatchChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<param_count::ParamCountChecker>(&mut context, semantic_model);
    run_checked::<check_return_count::CheckReturnCountChecker>(&mut context, semantic_model);
    run_checked::<incomplete_signature_doc::IncompleteSignatureDocChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<unnecessary_assert::UnnecessaryAssertChecker>(&mut context, semantic_model);
    run_checked::<unnecessary_if::UnnecessaryIfChecker>(&mut context, semantic_model);
    run_checked::<await_in_sync::AwaitInSyncChecker>(&mut context, semantic_model);
    run_checked::<access_invisible::AccessInvisibleChecker>(&mut context, semantic_model);
    run_checked::<cast_type_mismatch::CastTypeMismatchChecker>(&mut context, semantic_model);
    run_checked::<call_non_callable::CallNonCallableChecker>(&mut context, semantic_model);
    run_checked::<enum_value_mismatch::EnumValueMismatchChecker>(&mut context, semantic_model);
    run_checked::<attribute_check::AttributeCheckChecker>(&mut context, semantic_model);
    run_checked::<need_check_nil::NeedCheckNilChecker>(&mut context, semantic_model);
    run_checked::<unresolved_require::UnresolvedRequireChecker>(&mut context, semantic_model);
    run_checked::<require_module_visibility::RequireModuleVisibilityChecker>(
        &mut context,
        semantic_model,
    );
    run_checked::<undefined_field::UndefinedFieldChecker>(&mut context, semantic_model);
    run_checked::<check_export::CheckExportChecker>(&mut context, semantic_model);
    run_checked::<duplicate_field::DuplicateFieldChecker>(&mut context, semantic_model);
    run_checked::<duplicate_index::DuplicateIndexChecker>(&mut context, semantic_model);
    run_checked::<readonly::ReadOnlyChecker>(&mut context, semantic_model);
    run_checked::<check_field::CheckFieldChecker>(&mut context, semantic_model);
    run_checked::<discard_returns::DiscardReturnsChecker>(&mut context, semantic_model);
    run_checked::<global_non_module::GlobalInNonModuleChecker>(&mut context, semantic_model);
