...
use emmylua_parser::{LuaAst, LuaAstNode, LuaAstToken, LuaIndexExpr, LuaNameExpr, LuaNameToken};
use rowan::TextRange;

use crate::compilation::{
    SalsaDocOwnerKindSummary, SalsaDocOwnerSummary, SalsaDocVisibilityKindSummary,
};
...
fn check_name(context: &mut DiagnosticContext, model: &SemanticModel, name: LuaNameExpr) {
    let Some(semantic_decl) =
        model.find_decl_by_node(name.syntax().clone(), SemanticDeclLevel::default())
    else {
        return;
    };
    ...
    let Some(name_tk) = name.get_name_token() else {
        return;
    };
...
fn check_index(context: &mut DiagnosticContext, model: &SemanticModel, ix: LuaIndexExpr) {
    let Some(semantic_decl) =
        model.find_decl_by_node(ix.syntax().clone(), SemanticDeclLevel::default())
    else {
        return;
    };
    ...
    let Some(idx_tk) = ix.get_index_name_token() else {
        return;
    };
