...
fn check_class(context: &mut DiagnosticContext, model: &SemanticModel, tag: &LuaDocTagClass) {
    let Some(name) = tag.get_name_token().map(|t| t.get_name_text().to_string()) else { return };
    let db = model.salsa_db().read().unwrap_or_else(|e| e.into_inner());
    let Some(class_def) = db.doc().type_def_by_name(model.get_file_id(), &name) else { return };
    if !matches!(class_def.kind, crate::compilation::SalsaDocTypeDefKindSummary::Class) { return }
        ...
        if let Some(def) = db.doc().type_def_by_name(model.get_file_id(), &cur) {
            for offset in &def.super_type_offsets {
                if let Some(resolved) = db.doc().resolved_type_by_key(model.get_file_id(), *offset) {
                    if let crate::compilation::SalsaDocTypeLoweredKind::Name { name: super_name } = &resolved.lowered.kind {
                        if super_name.as_str() == &name {
                            let range = get_lint_range(tag).unwrap_or(tag.get_range());
                            context.add_diagnostic(DiagnosticCode::CircleDocClass, range,
                                t!("Circularly inherited classes.").to_string(), None);
                            return;
                        }
