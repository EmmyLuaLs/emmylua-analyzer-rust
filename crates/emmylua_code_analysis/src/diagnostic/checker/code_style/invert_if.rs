...
fn check_early_return(context: &mut DiagnosticContext, if_stat: &LuaIfStat) {
    let Some(else_clause) = if_stat.get_else_clause() else {
        return;
    };
    if if_stat.get_else_if_clause_list().next().is_some() {
        return;
    }
    let Some(if_block) = if_stat.get_block() else {
        return;
    };
    ...
    if exit_type.is_none() {
        return;
    }
    if exit_type == Some(Exit::Break) && !in_loop {
        return;
    }
    if block_ends_with_exit(&if_block) {
        return;
    }
    if !has_code_after(if_stat) {
        return;
    }
    if count_stmts(&if_block) < 3 {
        return;
    }
...
fn block_ends_with_exit(block: &LuaBlock) -> bool {
    block
        .get_stats()
        .last()
        .is_some_and(|s| matches!(s, LuaStat::ReturnStat(_) | LuaStat::BreakStat(_)))
}
