...
pub fn check(context: &mut DiagnosticContext, model: &SemanticModel) {
    ...
    for call in root.descendants::<LuaCallExpr>() {
        if call.is_assert() { check_assert(context, model, call); }
    }
}

fn check_assert(context: &mut DiagnosticContext, model: &SemanticModel, call: LuaCallExpr) {
    let Some(_) = call.get_parent::<LuaLocalStat>() else { return };
    ...
    match second {
        ...
        LuaExpr::NameExpr(name) => {
            let Some(name_text) = name.get_name_text() else { return };
            let Some(decl_tree) = model.decl_tree() else { return };
            let Some(decl) = decl_tree.decls.iter().find(|d| d.name.as_str() == name_text) else { return };
            if matches!(decl.kind, crate::compilation::SalsaDeclKindSummary::Local { .. }) { return }
        }
        _ => {}
    }
    context.add_diagnostic(
        DiagnosticCode::NonLiteralExpressionsInAssert, second.get_range(),
        t!("codestyle.NonLiteralExpressionsInAssert").to_string(), None,
    );
}
