...
fn check_assign(context: &mut DiagnosticContext, model: &SemanticModel, assign: &LuaAssignStat) {
    ...
    for var in vars {
        ...
        while let Some(expr) = cur {
            let Some(decl_id) = model.find_decl_by_node(
                expr.syntax().clone(),
...
fn check_readonly(
    ...
    let (file_id, offset) = match decl_id {
        ...
        LuaSemanticDeclId::Member(member_id) => {
            ...
            (member_id.file_id, member_id.get_syntax_id().get_range().start())
        }
        _ => return,
    };

    if model.decl_has_doc_property(file_id, offset, SalsaDocTagPropertyEntrySummary::Readonly) {
