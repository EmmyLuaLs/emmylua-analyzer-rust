...
use emmylua_parser::{BinaryOperator, LuaAstNode, LuaBinaryExpr, LuaCallExpr, LuaExpr, LuaIndexExpr};
...
fn check_call(context: &mut DiagnosticContext, model: &SemanticModel, call: LuaCallExpr) {
    ...
    let Ok(func) = model.infer_expr(prefix.clone()) else { return };
    if func.is_nullable() {
...
fn check_index(context: &mut DiagnosticContext, model: &SemanticModel, index: LuaIndexExpr) {
    let Some(prefix) = index.get_prefix_expr() else { return };
...
fn check_binary(context: &mut DiagnosticContext, model: &SemanticModel, binary: LuaBinaryExpr) {
    let Some(op_token) = binary.get_op_token() else { return };
    if !matches!(
        op_token.get_op(),
        BinaryOperator::OpAdd | BinaryOperator::OpSub | BinaryOperator::OpMul
            | BinaryOperator::OpDiv | BinaryOperator::OpMod
    ) {
        return;
    }
    let Some((left, right)) = binary.get_exprs() else { return };

    let Ok(left_type) = model.infer_expr(left.clone()) else { return };
    ...
    let Ok(right_type) = model.infer_expr(right.clone()) else { return };
