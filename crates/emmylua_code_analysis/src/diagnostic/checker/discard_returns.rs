...
pub fn check(context: &mut DiagnosticContext, model: &SemanticModel) {
    ...
    for stat in root.descendants::<LuaCallExprStat>() {
        let Some(call) = stat.get_call_expr() else { continue };
        let Some(prefix) = call.get_prefix_expr() else { continue };
        let range = prefix.syntax().text_range();

        let Ok(prefix_type) = model.infer_expr(prefix) else { continue };
