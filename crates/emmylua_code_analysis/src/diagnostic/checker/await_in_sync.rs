...
fn check_call_as_arg(context: &mut DiagnosticContext, model: &SemanticModel, call: LuaCallExpr) {
    ...
    for (i, (_, param_ty)) in func_info.params.iter().enumerate() {
        let Some(LuaType::DocFunction(f)) = param_ty else {
            continue;
        };
        ...
        let Some(arg_list) = call.get_args_list() else {
            continue;
        };
        ...
        let Some(arg) = arg_list.get_args().nth(arg_idx) else {
            continue;
        };

        let Ok(arg_type) = model.infer_expr(arg.clone()) else {
            continue;
        };
        let arg_is_async = match &arg_type {
            LuaType::DocFunction(f) => f.get_async_state() == AsyncState::Async,
            LuaType::Signature(sig) => model
                .get_signature(model.get_file_id(), sig.get_position())
