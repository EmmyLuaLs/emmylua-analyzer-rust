...
use crate::compilation::{SalsaDeclKindSummary, SalsaLocalAttributeSummary, SalsaUseSiteRoleSummary};
...
pub fn check(context: &mut DiagnosticContext, model: &SemanticModel) {
    ...
    for decl in &decl_tree.decls {
        ...
        let Some(refs) = model.decl_references(decl.id) else { continue };
        for r in &refs {
