...
use crate::{
    ...
    diagnostic::checker::humanize_lint_type, humanize_type, semantic::infer_call_expr_func,
};
...
fn check_call_expr(
    ...
    if !has_non_callable_member(&call_expr_type) {
        return Some(());
    }
...
fn has_non_callable_member(typ: &LuaType) -> bool {
    ...
    match typ {
        ...
        LuaType::Union(union) => union.into_vec().iter().any(has_non_callable_member),
        LuaType::MultiLineUnion(union) => union
            .get_unions()
            .iter()
            .any(|(t, _)| has_non_callable_member(t)),
        LuaType::TypeGuard(inner) => has_non_callable_member(inner),
        _ => true,
    }
}
...
fn collect_non_callable_union_types(
    ...
    match typ {
        ...
        LuaType::TypeGuard(inner) => {
            return collect_non_callable_union_types(db, cache, call_expr, inner);
        }
        _ => {}
    }
