...
fn check_call(context: &mut DiagnosticContext, model: &SemanticModel, call: LuaCallExpr) {
    ...
    if model.infer_call_expr_func(call, None).is_some() {
        return;
    }
    let Ok(prefix_type) = model.infer_expr(prefix.clone()) else {
        return;
    };
    if matches!(prefix_type, LuaType::Any | LuaType::Unknown) {
        return;
    }
    context.add_diagnostic(
        DiagnosticCode::CallNonCallable,
        prefix.get_range(),
        t!("%{name} is not callable", name = prefix.syntax().text()).to_string(),
        None,
    );
}
