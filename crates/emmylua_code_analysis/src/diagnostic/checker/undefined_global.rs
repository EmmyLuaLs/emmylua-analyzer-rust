...
fn collect_use_ranges(model: &SemanticModel) -> HashSet<TextRange> {
    let Some(decl_tree) = model.decl_tree() else { return HashSet::new() };
...
fn check_name(
    ...
    if use_ranges.contains(&name_range) { return }

    let Some(name_text) = name_expr.get_name_text() else { return };
    if name_text == "_" { return }

    // 全局声明存在？
    let db = model.salsa_db().read().unwrap_or_else(|e| e.into_inner());
    if db.types().global(model.get_file_id(), &name_text).is_some() { return }

    // 配置中的例外
    if context.config.global_disable_set.contains(name_text.as_str()) { return }
    if context.config.global_disable_glob.iter().any(|re| re.is_match(&name_text)) { return }

    // self 检查
    if name_text == "self" && is_self_valid(name_expr) { return }
...
fn is_self_valid(name_expr: LuaNameExpr) -> bool {
    ...
    for closure in closures {
        let Some(func_stat) = closure.get_parent::<LuaFuncStat>() else { continue };
        let Some(func_name) = func_stat.get_func_name() else { continue };
