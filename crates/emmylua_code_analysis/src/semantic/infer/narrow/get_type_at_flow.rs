...
enum Continuation {
    ...
    // Resume a recovered RHS fallback after proving the branch can reach the
    // assignment. Example: `x = config.missing` recovers `nil`, but an
    // impossible branch must not contribute that fallback to a merge.
    AssignmentFallback {
        walk: QueryWalk,
...
impl<'a> FlowTypeEngine<'a> {
        ...
        loop {
            step = match step {
                ...
                SchedulerStep::ResumeNext(query_result) => match stack.pop() {
                    ...
                    Some(Continuation::AssignmentFallback {
                        walk,
                        fallback_type,
                    }) => match query_result {
                        ...
                        Ok(FlowQueryResult::Type(_)) => Ok(self.finish_walk(walk, fallback_type)),
    ...
    fn finish_assignment_expr_type(
        ...
        if let Some(explicit_var_type) = explicit_var_type {
            if matches!(walk.query.mode, FlowMode::MergeBranch) {
                // The annotation still supplies the assignment source type, but
                // this branch must first prove it can reach the assignment.
                let subquery = walk
                    .query
                ...
                return Ok(SchedulerStep::StartQuery {
                    ...
                    continuation: Some(Continuation::AssignmentAntecedent {
                        walk,
                        antecedent_flow_id,
                        explicit_var_type: Some(explicit_var_type),
                        expr_type,
                        reuse_antecedent_narrowing: true,
                    }),
                });
            }
    ...
    fn finish_assignment_expr_error(
        ...
        if let Some(fallback_type) = fallback_type {
            if matches!(walk.query.mode, FlowMode::MergeBranch) {
                ...
                return Ok(SchedulerStep::StartQuery {
                    query,
                    continuation: Some(Continuation::AssignmentFallback {
                        walk,
                        fallback_type,
                    }),
                });
    ...
    fn step_tag_cast(
        ...
        let mode = match walk.query.mode {
            ...
            // IgnoreConditions belongs to the assignment source retry. A tag
            // cast dependency is its own point query, so it should use normal
            // condition handling unless this is still a merge contribution.
    ...
    fn apply_condition_action(
        ...
        match action {
            ...
            ConditionFlowAction::NeedCorrelated(pending_correlated_condition) => {
                let mode = match walk.query.mode {
                    FlowMode::MergeBranch => FlowMode::MergeBranch,
                    // IgnoreConditions belongs to the assignment source retry.
                    // Correlated-condition dependencies need normal condition
                    // handling unless this is still a merge contribution.
                    FlowMode::Normal | FlowMode::IgnoreConditions => FlowMode::Normal,
                };
