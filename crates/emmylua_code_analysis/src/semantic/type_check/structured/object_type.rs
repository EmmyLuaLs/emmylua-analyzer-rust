...
use crate::{
    DbIndex, LuaArrayType, LuaMemberIndexItem, LuaMemberKey, LuaMemberOwner, LuaObjectType,
    LuaTupleType, LuaType, LuaTypeDeclId, TypeSubstitutor, instantiate_type_generic,
...
pub(super) fn relate_object_to_array(
    ...
    for (key, source_type) in source_object.get_fields() {
        ...
        relater
            ...
            .map_err(|failure| {
                failure.map_mismatch(|mismatch| {
                    mismatch.at(TypePathSegment::Member(key.clone()), source, target)
                })
            })?;
        checked = true;
        relater.note_progress();
    ...
    for (source_key, source_type) in source_object.get_index_access() {
        ...
        relater
            ...
            .map_err(|failure| {
                failure.map_mismatch(|mismatch| {
                    mismatch.at(TypePathSegment::Index(source_key.clone()), source, target)
                })
            })?;
        checked = true;
        relater.note_progress();
    ...
    for (source_key_type, source_type) in source_object.get_index_access() {
        ...
        relater
            ...
            .map_err(|failure| {
                failure.map_mismatch(|mismatch| {
                    mismatch.at(
                        TypePathSegment::Index(source_key_type.clone()),
                        source,
                        target,
                    )
                })
            ...
            let Some(source_member_type) = source_member_type else {
                ...
                return relater.unrelated(|| {
                    TypeMismatch::new(
                        source,
                        target,
                        TypeMismatchKind::MissingMember {
            ...
            if let Err(failure) = field_result {
                if relater.is_explain() {
                    return Err(failure.map_mismatch(|mismatch| {
                        mismatch.at(TypePathSegment::Member(target_key.clone()), source, target)
                    }));
                }
                return Err(failure);
            }
    ...
    let Some(source_member_type) = source_member_type else {
        ...
        return relater.unrelated(|| {
            TypeMismatch::new(
                source,
                target,
                TypeMismatchKind::MissingMember { key: key.clone() },
            )
        });
    };
...
pub(in crate::semantic::type_check) fn relate_target_intersection_index_obligations(
    ...
    source: &LuaType,
    target: &LuaType,
    intersection: &crate::LuaIntersectionType,
) -> RelationResult {
    for member in intersection.get_types() {
        match member {
...
fn visit_declared_type_members(
    ...
    let (type_id, substitutor) = match declared_type {
    ...
    if !has_supers && seen_keys.is_empty() {
        visit_member_items(db, &owner, |key, item| {
            let Ok(mut member_type) = item.resolve_type(db) else {
                return Ok(());
            };
            if let Some(substitutor) = &substitutor {
                member_type = instantiate_type_generic(db, &member_type, substitutor);
            }
