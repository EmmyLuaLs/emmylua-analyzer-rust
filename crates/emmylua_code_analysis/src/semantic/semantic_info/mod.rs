...
fn infer_local_name_initializer_type(
    db: &DbIndex,
    cache: &mut LuaInferCache,
    token: LuaSyntaxToken,
) -> Option<LuaType> {
    let local_name = LuaLocalName::cast(token.parent()?)?;
    let local_stat = local_name.get_parent::<LuaLocalStat>()?;
    let local_idx = local_stat
        .get_local_name_list()
        .position(|name| name.get_position() == local_name.get_position())?;
    let expr = local_stat.get_value_exprs().nth(local_idx)?;

    infer_expr(db, cache, expr).ok()
}
