...
use crate::{
    GenericTplId, LuaArrayType, LuaGenericType, LuaIntersectionType, LuaMemberKey, LuaMappedType,
...
impl GenericBindings {
    pub fn new() -> Self {
        Self { map: HashMap::new(), self_type: None, max_depth: 8 }
    }
    ...
    pub fn from_explicit_args(type_args: &[LuaType]) -> Self {
        ...
        for (i, ty) in type_args.iter().enumerate() {
            bindings.map.insert(GenericTplId::Func(i as u32), ty.clone());
        }
        bindings
    }
    ...
    pub fn from_type_args(type_args: &[LuaType]) -> Self {
        let mut bindings = Self::new();
        for (i, ty) in type_args.iter().enumerate() {
            bindings.map.insert(GenericTplId::Type(i as u32), ty.clone());
        }
        bindings
    }
...
impl<'a> SubstitutionContext<'a> {
    fn substitute_inner(&mut self, ty: &LuaType, depth: u32) -> LuaType {
        ...
        match ty {
            // ── 直接替换 ──
            LuaType::TplRef(tpl) | LuaType::ConstTplRef(tpl) => {
                self.bindings.get(tpl.get_tpl_id()).cloned().unwrap_or_else(|| ty.clone())
            }
            LuaType::StrTplRef(str_tpl) => {
                if let Some(bound) = self.bindings.get(str_tpl.get_tpl_id()) {
                    let resolved = resolve_str_tpl(
                        str_tpl.get_prefix(), str_tpl.get_suffix(), &bound,
                    );
            ...
            LuaType::SelfInfer => {
                self.bindings.self_type().cloned().unwrap_or_else(|| ty.clone())
            }
            ...
            LuaType::Array(arr) => {
                ...
                if &new_base == base { ty.clone() }
                else { LuaType::Array(LuaArrayType::new(new_base, arr.get_len().clone()).into()) }
            }
            LuaType::Union(u) => {
                let old = u.into_vec();
                let new: Vec<LuaType> = old.iter()
            ...
            LuaType::Tuple(tuple) => {
                ...
                let new_types: Vec<LuaType> = types.iter()
                    .map(|t| {
                    ...
                    .collect();
                if !changed { ty.clone() }
                else { LuaType::Tuple(LuaTupleType::new(new_types, LuaTupleStatus::DocResolve).into()) }
            }
            LuaType::TableGeneric(params) => {
                let new_params: Vec<LuaType> = params.iter()
                    .map(|p| self.substitute_inner(p, depth + 1))
                    .collect();
                if new_params.as_slice() == params.as_slice() { ty.clone() }
                else { LuaType::TableGeneric(Arc::new(new_params)) }
            }
            LuaType::Mapped(mapped) => {
                ...
                if new_value == mapped.value { ty.clone() }
                else {
                    LuaType::Mapped(LuaMappedType::new(
                        mapped.param.clone(), new_value,
                        mapped.is_readonly, mapped.is_optional,
                    ).into())
                }
            }
    ...
    fn substitute_generic(&mut self, generic: &Arc<LuaGenericType>, depth: u32) -> LuaType {
        ...
        let new_params: Vec<LuaType> = old_params.iter()
            .map(|p| self.substitute_inner(p, depth + 1))
            .collect();
    ...
    fn substitute_object(&mut self, obj: &Arc<LuaObjectType>, depth: u32) -> LuaType {
        ...
        for (key, val) in fields.iter() {
            ...
            if &new_val != val { changed = true; }
            new_fields.push((key.clone(), new_val));
        }
        if !changed { return LuaType::Object(obj.clone()); }
        LuaType::Object(
    ...
    fn substitute_variadic(&mut self, variadic: &Arc<VariadicType>, depth: u32) -> LuaType {
        match variadic.as_ref() {
            VariadicType::Base(base) => {
                let new_base = self.substitute_inner(base, depth + 1);
                if &new_base == base { LuaType::Variadic(variadic.clone()) }
                else { LuaType::Variadic(VariadicType::Base(new_base).into()) }
            }
            VariadicType::Multi(types) => {
                ...
                let new_types: Vec<LuaType> = types.iter()
                    ...
                    .collect();
                if !changed { LuaType::Variadic(variadic.clone()) }
                else { LuaType::Variadic(VariadicType::Multi(new_types).into()) }
            }
        }
    }
}
