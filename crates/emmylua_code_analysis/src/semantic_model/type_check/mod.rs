...
fn lookup_type_def(
    ctx: &CheckContext,
    name: &str,
) -> Option<(FileId, crate::compilation::SalsaDocTypeDefSummary)> {
    let db = ctx.db?;
...
fn resolve_alias_target_type(
    ctx: &CheckContext,
    file_id: FileId,
    key: &crate::compilation::SalsaDocTypeNodeKey,
) -> Option<LuaType> {
    let db = ctx.db?;
...
fn resolve_doc_type_ref(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    ty_ref: &crate::compilation::SalsaDocTypeRef,
) -> Option<LuaType> {
    match ty_ref {
        crate::compilation::SalsaDocTypeRef::Node(key) => {
