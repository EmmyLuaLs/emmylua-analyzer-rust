...
impl GenericChecker {
    ...
    fn from_call_class_generics(
        ...
        use crate::compilation::SalsaMemberRootSummary;
    ...
    fn build_class_checker(
        class_name: &str,
        db: impl std::ops::Deref<Target = crate::compilation::SalsaSummaryDatabase>,
        file_id: FileId,
        call_expr: &LuaCallExpr,
        sig: &SalsaSignatureExplainSummary,
        infer: &InferQuery,
...
struct RawGeneric {
    name: String,
    c_lt: Option<crate::compilation::SalsaDocTypeLoweredNode>,
    d_lt: Option<crate::compilation::SalsaDocTypeLoweredNode>,
}

/// 解析约束：检测泛型引用（`Infer { "T" }` 或 `Name { "T" }` 且 T 在 raw 中）。
/// 返回 (resolved_type, is_rigid=true 表示引用另一个泛型)
fn resolve_constraint(
    lt: Option<&crate::compilation::SalsaDocTypeLoweredNode>,
    raw: &[RawGeneric],
) -> (Option<LuaType>, bool) {
...
fn generic_ref_name(
    lt: &crate::compilation::SalsaDocTypeLoweredNode,
    raw: &[RawGeneric],
) -> Option<String> {
    match &lt.kind {
...
fn extract_infer_names(
    node: &crate::compilation::SalsaDocTypeLoweredNode,
    known_generics: &[String],
) -> Vec<String> {
...
fn resolve_arg_type_annotation(
    ...
    use crate::compilation::SalsaDocOwnerKindSummary;
