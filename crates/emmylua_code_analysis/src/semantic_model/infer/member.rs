...
fn infer_member_impl_inner(
    ...
    match prefix_type {
        ...
        LuaType::Object(obj) => {
            ...
            // TypeKey key: 检查是否有字段的 key 类型兼容
            if let LuaMemberKey::TypeKey(key_ty) = member_key {
        ...
        LuaType::Array(arr) => {
            if matches!(
                member_key,
                LuaMemberKey::Integer(_) | LuaMemberKey::TypeKey(_)
...
fn infer_custom_type_member(
    ...
    // 2. TypeKey → 模糊匹配（如 obj[key] where key: string）
    if let LuaMemberKey::TypeKey(key_ty) = member_key {
...
fn is_integer_key(key: &LuaMemberKey) -> bool {
    matches!(key, LuaMemberKey::Integer(_))
        || matches!(
            key,
            LuaMemberKey::TypeKey(ty) if ty.is_integer()
        )
}
...
fn member_key_to_type(key: &LuaMemberKey) -> LuaType {
    match key {
        ...
        LuaMemberKey::TypeKey(ty) => ty.clone(),
