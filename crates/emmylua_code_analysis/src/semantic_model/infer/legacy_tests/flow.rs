...
mod test {
    ...
    fn last_name_expr_type(ws: &VirtualWorkspace, file_id: FileId, name: &str) -> LuaType {
        ...
        semantic_model
            .semantic_info(name_expr.syntax().clone().into())
    ...
    fn test_issue_1028_maxwellhome_like_large_array_builds_semantic_model() {
        ...
        let info = semantic_model
            .semantic_info(token.syntax().clone().into())
...
n = n + 1
    ...
    fn test_issue_600() {
        let mut ws = VirtualWorkspace::new();
        // New semantics: `a` is Test2 runtime table and `a.test` is known `string[]`;
        // Under strict array indexing elements are nullable; current flow does not yet narrow truthiness along class-field array subscripts.
        assert!(!ws.has_no_diagnostic(
            DiagnosticCode::NeedCheckNil,
            r#"
            ---@class Test2
