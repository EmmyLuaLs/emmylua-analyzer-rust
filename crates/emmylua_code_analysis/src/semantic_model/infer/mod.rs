...
use crate::compilation::{
    ...
    SalsaDocTypeLoweredKind, SalsaDocTypeLoweredNode, SalsaDocTypeRef,
...
impl<'db> InferQuery<'db> {
    ...
    pub(crate) fn sig_query(&self) -> SigQuery<'_> {
    ...
    pub fn infer_expr(&self, expr: LuaExpr) -> InferResult {
        ...
        if let Some(cached) = self.cache.borrow().get(&syntax_id) {
            if let LuaExpr::NameExpr(_name) = expr {}
            return cached;
        }
    ...
    fn resolve_decl_type_depth(
        ...
        // Priority 0.6: value expression is a LiteralExpr → infer literal type
        // (e.g., `local x = "hello"` → string, `local x = 42` → integer)
        if let Some(sid) = &decl_type.value_expr_syntax_id {
            if sid.kind == LuaSyntaxKind::LiteralExpr {
                if let Some(lit_type) = self.infer_literal_from_syntax_id(sid) {
                    return Some(lit_type);
                }
            }
        }
        // Priority 0.7: value expression is a NameExpr → trace through to the name's type
    ...
    fn infer_literal_from_syntax_id(&self, sid: &SalsaSyntaxIdSummary) -> Option<LuaType> {
        for lit in self.root.descendants::<LuaLiteralExpr>() {
            if lit.get_position() == sid.start_offset {
                return self.infer_literal_type(lit);
            }
        }
        None
    }

    fn infer_literal_type(&self, lit: LuaLiteralExpr) -> Option<LuaType> {
        ...
        match token {
            ...
            LuaLiteralToken::Number(n) => match n.get_number_value() {
                ...
                NumberResult::Float(f) => Some(LuaType::FloatConst(f)),
    ...
    fn resolve_single_named_type(
        ...
        let (type_def, _def_file_id) = tq.get_def_with_file(name.as_str())?;
    ...
    fn resolve_doc_type_for_decl(
        ...
        let decl_pos: TextSize = decl_id.as_text_size();
            ...
            if let Some(owner_off) = tag.owner.syntax_offset {
                ...
                if decl_tree.decls.iter().any(|d| {
                    d.start_offset >= owner_off
                        && d.start_offset < decl_pos + TextSize::from(50u32)
                        && d.name == decl_name
                }) {
    ...
    fn type_decl_id_from_visibility(
        ...
        match visibility {
            SalsaDocVisibilityKindSummary::Private => LuaTypeDeclId::file(self.file_id, name),
    ...
    fn try_narrow_by_flow(&self, base_type: LuaType, name: &str, offset: TextSize) -> LuaType {
        let db = self.read_db();
        let decl_tree = db.file().decl_tree(self.file_id);
        narrow::narrow_local_at_point(
            &db,
            self.file_id,
            &self.root,
            offset,
        ...
        if let Some(name_info) = db.types().name(self.file_id, name_expr.get_position()) {
            if let Some(ref decl_type) = name_info.decl_type {
                if let Some(ty) = self.resolve_decl_type(&db, decl_type.clone()) {
                    return Ok(self.try_narrow_by_flow(ty, &name, name_expr.get_position()));
                }
            }
            if let SalsaNameUseResolutionSummary::LocalDecl(decl_id) =
                &name_info.name_use.resolution
            ...
            if let Some(lex_use) = db.lexical().use_at(self.file_id, name_expr.get_position()) {
                if let SalsaLexicalUseSummary::Name { resolution, .. } = &lex_use {
                    if let SalsaNameUseResolutionSummary::LocalDecl(decl_id) = resolution {
                        if let Some(dt) = db.types().decl(self.file_id, *decl_id) {
                            if let Some(ty) = self.resolve_decl_type(&db, dt) {
                                return Ok(self.try_narrow_by_flow(
                                    ty,
                                    &name,
