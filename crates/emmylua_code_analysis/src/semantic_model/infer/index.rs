...
pub(super) fn infer_index_expr(infer: &InferQuery, index_expr: LuaIndexExpr) -> InferResult {
    ...
    if let Some(member_type_info) = db
        .types()
        .member_use(infer.file_id, member_expr.get_position())
    {
        if let Some(ty) = infer.resolve_member_type(&db, &member_type_info) {
            return Ok(ty);
        }
    }
    ...
    match name.as_str() {
        ...
        _ => {
            ...
            let type_id = if matches!(type_def.visibility, SalsaDocVisibilityKindSummary::Private) {
                LuaTypeDeclId::file(file_id, name.as_str())
            } else {
...
fn member_key_from_expr(
    ...
    match &index_key {
        LuaIndexKey::Name(name) => Ok(LuaMemberKey::Name(SmolStr::new(name.get_name_text()))),
        LuaIndexKey::String(s) => Ok(LuaMemberKey::Name(SmolStr::new(s.get_value()))),
        ...
        LuaIndexKey::Expr(expr) => {
            ...
            Ok(match key_type {
                ...
                _ => LuaMemberKey::TypeKey(key_type),
            })
        }
    }
}
