...
pub(super) fn infer_binary_expr(
    ...
    let op = binary_expr
        ...
        .get_op();
    let (left_expr, right_expr) = binary_expr
        .get_exprs()
        .ok_or(InferFailReason::None)?;
...
fn infer_cmp(left: &LuaType, right: &LuaType, op: BinaryOperator) -> InferResult {
    ...
    // Table 常量 eq/ne
    match (left, right) {
        (LuaType::TableConst(a), LuaType::TableConst(b)) => match op {
            BinaryOperator::OpEq => return Ok(LuaType::BooleanConst(a == b)),
            BinaryOperator::OpNe => return Ok(LuaType::BooleanConst(a != b)),
            _ => return Ok(LuaType::Boolean),
        },
...
fn narrow_false_or_nil(ty: &LuaType) -> LuaType {
    match ty {
        ...
        LuaType::Boolean => LuaType::Union(
            LuaUnionType::Nullable(LuaType::BooleanConst(false)).into(),
        ),
        LuaType::Union(u) => {
