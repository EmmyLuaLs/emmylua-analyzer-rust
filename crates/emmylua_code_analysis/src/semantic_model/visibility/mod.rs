...
fn check_type_matches_owner(ty: &LuaType, owner: &LuaSemanticDeclId, is_protected: bool) -> bool {
    match (ty, owner) {
        (LuaType::Def(def_id), LuaSemanticDeclId::TypeDecl(type_id)) => {
            ...
            if is_protected {
                return type_check::is_sub_type_of_name(def_id, type_id);
            }
            false
        }
        _ => false,
...
fn check_private_name_pattern(_emmyrc: &Arc<Emmyrc>, property_owner: &LuaSemanticDeclId) -> bool {
...
pub fn check_module_visibility(
    ...
    match module_visibility {
        ...
        crate::ModuleVisibility::Internal => match (current_workspace_id, module_workspace_id) {
