...
use emmylua_parser::{
    BinaryOperator, LuaAssignStat, LuaAst, LuaAstNode, LuaBlock, LuaCallExprStat,
...
use super::{
    ...
    stats::{
        ...
        finish_entered_loop_post_flow, get_local_decl_ids, get_var_decl_ids, get_var_member_ids,
...
impl<'a, 'b> BindEngine<'a, 'b> {
    ...
    fn evaluate_node(&mut self, node: LuaAst, current: FlowId) -> Step {
        match node {
            ...
            LuaAst::LuaAssignStat(assign_stat) => {
                ...
                // Collect trailing/inline comments first and put them at the front of pending;
                // Seq pops from the tail, so they are processed after all variable/value expressions,
                // applying `--[[@as T]]` to the assignment's right-hand side.
                for child in assign_stat.syntax().children() {
                    if let Some(ast) = LuaAst::cast(child)
                        && matches!(ast, LuaAst::LuaComment(_))
                    {
            ...
            LuaAst::LuaLocalStat(local_stat) => {
                ...
                for i in 0..min_len {
                    ...
                    let value = &values[i];
                    let decl_id = SemanticId::decl(self.binder.file_id, name.get_range());
            ...
            LuaAst::LuaBreakStat(_break_stat) => {
            ...
            LuaAst::LuaContinueStat(_continue_stat) => {
    ...
    fn evaluate_for_stat(&mut self, for_stat: LuaForStat, current: FlowId) -> Step {
        ...
        let stop_expr = iter_exprs.get(1).cloned();
    ...
    fn resume(&mut self, continuation: Continuation, value: FlowId) -> Step {
        match continuation {
            ...
            Continuation::LocalDone {
                ...
                let local_names = local_stat.get_local_name_list().collect::<Vec<_>>();
                let values = local_stat.get_value_exprs().collect::<Vec<_>>();
                let decl_ids = get_local_decl_ids(self.binder, &local_names);
                for (decl_id, value) in decl_ids.iter().zip(values.iter()) {
                    if let Some(decl_id) = decl_id {
                        self.binder.add_effect(
                            local_flow_id,
                            FlowEffect::AssignDecl {
                                decl: decl_id.clone(),
                                value_syntax: value.get_syntax_id(),
                            },
                        );
                    }
                }
            ...
            Continuation::AssignDone {
                ...
                let (vars, values) = assign_stat.get_var_and_expr_list();
                let decl_ids = get_var_decl_ids(self.binder, &vars);
                for (i, decl_id) in decl_ids.iter().enumerate() {
                    if let Some(decl_id) = decl_id
                        && let Some(value) = values.get(i).or_else(|| values.last())
                    {
                        self.binder.add_effect(
                            flow_id,
                            FlowEffect::AssignDecl {
                ...
                let member_ids = get_var_member_ids(self.binder, &vars);
                for (i, member_id) in member_ids.iter().enumerate() {
                    if let Some((owner, key, member)) = member_id
                        && let Some(value) = values.get(i).or_else(|| values.last())
                    {
                        self.binder.add_effect(
                            flow_id,
            ...
            Continuation::ThenForNode {
                ...
                // Build a truthy guard on the loop body path for a dynamic numeric for
                // upper bound (e.g. `for i=1,#calls`): `#calls` truthy means calls is not falsy (false/nil removed).
                let body_current = if let Some(stop_expr) = stop_expr {
                    let guard = self
                        .binder
                        .create_node(FlowNodeKind::TrueCondition(stop_expr.to_ptr()));
                    self.binder.add_effect(
                        guard,
                        FlowEffect::Guard {
                            cond: stop_expr.to_ptr(),
                            truthy: true,
                        },
                    );
                    self.binder.add_antecedent(guard, pre_label);
                    let body_label = self.binder.create_branch_label();
                    self.binder.add_antecedent(body_label, guard);
                    body_label
                } else {
                    pre_label
                };
                ...
                self.binder.add_antecedent(for_node, body_current);
                match block {
