        ...
        match self {
            LuaType::Variadic(variadic) => match variadic.as_ref() {
                ...
                VariadicType::Multi(types) => {
                    ...
                    match last_type.get_result_slot_type(offset) {
                        Some(ty) => Some(ty),
                        // Finite multiple return: append nil beyond declared slots, matching Lua semantics.
                        None if !matches!(last_type, LuaType::Variadic(_)) => Some(LuaType::Nil),
                        None => None,
                    }
                }
            },
