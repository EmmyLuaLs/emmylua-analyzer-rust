...
use crate::{
    ...
    SalsaScopeKindSummary, SalsaScopeSummary, SalsaSummaryDatabase, SalsaSyntaxIdSummary, TypeOps,
...
fn infer_compilation_named_type(db: &DbIndex, file_id: FileId, name: &str) -> LuaType {
    if let Some(type_def) = db.get_summary_db().doc().type_def_by_name(file_id, name) {
        let workspace_id = db
            .resolve_workspace_id(file_id)
            .unwrap_or(WorkspaceId::MAIN);
...
pub(crate) struct CompilationDeclIndex<'a> {
    summary: &'a SalsaSummaryDatabase,
}

impl<'a> CompilationDeclIndex<'a> {
    pub fn new(summary: &'a SalsaSummaryDatabase) -> Self {
        Self { summary }
    }
