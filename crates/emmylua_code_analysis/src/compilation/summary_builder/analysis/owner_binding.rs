...
use emmylua_parser::{
    LuaAst, LuaAstNode, LuaAstToken, LuaChunk, LuaClosureExpr, LuaComment, LuaExpr, LuaVarExpr,
};
use rowan::TextSize;
...
pub fn analyze_doc_owner_binding_summary(
    ...
    for comment in chunk.descendants::<LuaComment>() {
        ...
        for target in build_targets(owner, decl_tree, properties, signatures) {
...
fn build_targets(
    ...
    match owner {
        LuaAst::LuaLocalStat(local_stat) => local_stat
            ...
            .filter_map(|local_name| {
                ...
                decl_tree
                    .decls
                    .iter()
                    .find(|decl| decl.start_offset == name_token.get_position())
        ...
        LuaAst::LuaAssignStat(assign_stat) => {
            ...
            vars.iter()
                .filter_map(|var| resolve_var_target(Some(var), decl_tree))
                .collect()
        }
