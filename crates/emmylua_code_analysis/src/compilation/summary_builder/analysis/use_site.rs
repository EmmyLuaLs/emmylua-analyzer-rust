...
pub fn analyze_use_site_summary(
    ...
    let names: Vec<SalsaNameUseSummary> = chunk
        .descendants::<LuaNameExpr>()
        .filter_map(|name_expr| build_name_use(decl_tree, name_expr))
        .collect();
    let members: Vec<SalsaMemberUseSummary> = chunk
    ...
    use super::super::build_lookup_buckets;

    let by_name_offset = build_lookup_buckets(
        names
            .iter()
            .enumerate()
            .map(|(i, n)| (n.syntax_offset, i))
            .collect(),
    );
    let by_member_offset = build_lookup_buckets(
        members
            .iter()
            .enumerate()
            .map(|(i, m)| (m.syntax_offset, i))
            .collect(),
    );
