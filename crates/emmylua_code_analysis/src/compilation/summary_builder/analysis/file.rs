...
impl SummaryFileAnalysis {
    fn new(file_id: FileId, chunk: LuaChunk) -> Self {
        Self {
            ...
            use_sites: SalsaUseSiteIndexSummary {
                ...
                members: Vec::new(),
                calls: Vec::new(),
                by_name_offset: Vec::new(),
                by_member_offset: Vec::new(),
            },
            flow: SalsaFlowSummary {
                branch_count: 0,
                loop_count: 0,
    ...
    fn run_signature_stage(&mut self) {
        self.signatures = analyze_signature_summary(self.file_id, self.chunk.clone(), &self.doc);
    }
    ...
    fn finish(self) -> SalsaFileSummary {
        let is_meta = self.doc.tags.iter().any(|tag| {
            use crate::compilation::SalsaDocTagDataSummary;
            matches!(tag.data,
                SalsaDocTagDataSummary::Name(ref n) | SalsaDocTagDataSummary::Content(ref n)
                if n.as_ref().map(|s| s.as_str() == "meta").unwrap_or(false))
        });
        SalsaFileSummary {
