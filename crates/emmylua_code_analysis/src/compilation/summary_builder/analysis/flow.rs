...
use emmylua_parser::{
    ...
    LuaLocalFuncStat, LuaStat, LuaSyntaxId,
};
use rowan::TextSize;

use super::super::{
    ConditionNodeId, SalsaFlowBlockOwnerKindSummary, SalsaFlowBlockSummary,
    SalsaFlowBranchClauseKindSummary, SalsaFlowBranchClauseSummary, SalsaFlowBranchSummary,
    SalsaFlowBreakSummary, SalsaFlowConditionKindSummary, SalsaFlowConditionSummary,
    SalsaFlowContinueSummary, SalsaFlowGotoSummary, SalsaFlowLabelSummary,
    SalsaFlowLoopKindSummary, SalsaFlowLoopSummary, SalsaFlowReturnSummary,
...
pub fn analyze_flow_summary(chunk: LuaChunk) -> SalsaFlowSummary {
    ...
    // Pre-collect cast positions
    let mut cast_positions: Vec<(TextSize, Option<TextSize>)> = Vec::new();
    for node in chunk.descendants::<LuaAst>() {
        if let LuaAst::LuaDocTagCast(cast) = node {
            let cast_pos = cast.get_position();
            let block = parent_block_offset_of(&cast);
            cast_positions.push((cast_pos, block));
        }
    }
    ...
    for node in chunk.descendants::<LuaAst>() {
        match node {
            ...
            LuaAst::LuaContinueStat(continue_stat) => {
                collector.continues.push(SalsaFlowContinueSummary {
                    syntax_offset: continue_stat.get_position(),
                    block_offset: parent_block_offset_of(&continue_stat),
                });
            }
    ...
    // Populate cast_offsets for each block
    for (cast_pos, block_opt) in &cast_positions {
        if let Some(block_offset) = block_opt {
            if let Some(block) = collector
                .blocks
                .iter_mut()
                .find(|b| b.syntax_offset == *block_offset)
            {
                block.cast_offsets.push(*cast_pos);
            }
        }
    }
...
struct FlowSummaryCollector {
    ...
    condition_ids: HashMap<(TextSize, TextSize), ConditionNodeId>,
    ...
    continues: Vec<SalsaFlowContinueSummary>,
...
impl FlowSummaryCollector {
    fn finish(self) -> SalsaFlowSummary {
        SalsaFlowSummary {
            ...
            continue_count: self.continues.len(),
...
fn build_statement_summary(block_offset: TextSize, stat: LuaStat) -> SalsaFlowStatementSummary {
    ...
    let kind = match stat {
        ...
        LuaStat::ContinueStat(_) => SalsaFlowStatementKindSummary::Continue,
