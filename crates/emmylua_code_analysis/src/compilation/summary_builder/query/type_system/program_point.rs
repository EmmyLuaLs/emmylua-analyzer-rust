...
fn build_generic_arg_type_map(
    ...
    generic_params
        ...
        .filter_map(|(index, param)| {
            ...
            match arg {
                Some(SalsaDocTypeRef::Node(offset)) => Some((param.name.clone(), vec![*offset])),
                Some(SalsaDocTypeRef::Incomplete) => param
                    .default_type_offset
                    .map(|offset| (param.name.clone(), vec![offset])),
                None => param
                    .default_type_offset
                    .map(|offset| (param.name.clone(), vec![offset])),
            }
...
fn build_default_generic_arg_type_map(
    generic_params: &[crate::SalsaDocGenericParamSummary],
) -> BTreeMap<SmolStr, Vec<SalsaDocTypeNodeKey>> {
    build_generic_arg_type_map(generic_params, &[])
}
...
fn collect_index_access_member_keys_from_named_type(
    ...
    let result = doc
        ...
        .map(|type_def| match type_def.kind {
            SalsaDocTypeDefKindSummary::Alias => type_def
                .value_type_offset
        ...
        .find(|doc_type| doc_type.node_key() == type_offset)
    {
        ...
        match &doc_type.kind {
            ...
            SalsaDocTypeKindSummary::Generic {
                ...
                if let Some(generic_args) = generic_args
                    && let Some(base_offset) = base_type_offset
                {
                    ...
                    candidates.extend(collect_member_candidates_from_generic_instance(
                        SalsaDocTypeRef::Node(*base_offset),
                        arg_types,
                        member_name,
                        property_query_index,
                        chunk,
...
fn substitute_type_offsets_shallow(
    ...
    match &doc_type.kind {
        SalsaDocTypeKindSummary::Infer { generic_name } => generic_name
            ...
            .and_then(|name| generic_args.get(name))
            .cloned()
            .unwrap_or_else(|| vec![type_offset]),
        SalsaDocTypeKindSummary::Name { name } => name
            .as_ref()
            .and_then(|name| generic_args.get(name))
            .cloned()
