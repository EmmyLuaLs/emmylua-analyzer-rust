...
pub struct SalsaFlowExactLookupIndex {
    ...
    pub continues: Vec<SalsaFlowContinueSummary>,
    pub gotos: Vec<SalsaFlowGotoSummary>,
    pub labels: Vec<SalsaFlowLabelSummary>,
    by_block_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_condition_offset: Vec<SalsaLookupBucket<ConditionNodeId>>,
    by_branch_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_loop_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_return_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_break_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_continue_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_goto_offset: Vec<SalsaLookupBucket<TextSize>>,
    by_label_offset: Vec<SalsaLookupBucket<TextSize>>,
}

pub fn build_flow_exact_lookup_index(flow: &SalsaFlowSummary) -> SalsaFlowExactLookupIndex {
    let blocks = flow.blocks.clone();
    let conditions = flow.conditions.clone();
    let branches = flow.branches.clone();
    let loops = flow.loops.clone();
    let returns = flow.returns.clone();
    let breaks = flow.breaks.clone();
    let continues = flow.continues.clone();
    let gotos = flow.gotos.clone();
    let labels = flow.labels.clone();

    SalsaFlowExactLookupIndex {
        by_block_offset: build_lookup_buckets(
            blocks
                .iter()
                .enumerate()
        ...
        by_condition_offset: build_lookup_buckets(
            conditions
                .iter()
                .enumerate()
                .map(|(index, condition)| (condition.node_offset, index))
                .collect(),
        ),
        by_branch_offset: build_lookup_buckets(
            branches
                .iter()
                .enumerate()
                .map(|(index, branch)| (branch.syntax_offset, index))
                .collect(),
        ),
        by_loop_offset: build_lookup_buckets(
            loops
                .iter()
                .enumerate()
                .map(|(index, loop_summary)| (loop_summary.syntax_offset, index))
                .collect(),
        ),
        by_return_offset: build_lookup_buckets(
            returns
                .iter()
                .enumerate()
                .map(|(index, return_summary)| (return_summary.syntax_offset, index))
                .collect(),
        ),
        by_break_offset: build_lookup_buckets(
            breaks
                .iter()
                .enumerate()
                .map(|(index, break_summary)| (break_summary.syntax_offset, index))
                .collect(),
        ),
        by_continue_offset: build_lookup_buckets(
            continues
                .iter()
                .enumerate()
                .map(|(index, continue_summary)| (continue_summary.syntax_offset, index))
                .collect(),
        ),
        by_goto_offset: build_lookup_buckets(
            gotos
                .iter()
                .enumerate()
                .map(|(index, goto_summary)| (goto_summary.syntax_offset, index))
                .collect(),
        ),
        by_label_offset: build_lookup_buckets(
            labels
                .iter()
                .enumerate()
                .map(|(index, label_summary)| (label_summary.syntax_offset, index))
                .collect(),
        ),
        blocks,
        conditions,
...
pub fn find_flow_block_in_index(
    index: &SalsaFlowExactLookupIndex,
    block_offset: TextSize,
) -> Option<SalsaFlowBlockSummary> {
    find_bucket_indices(&index.by_block_offset, &block_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.blocks[entry].clone())
}

pub fn find_flow_condition_in_index(
    index: &SalsaFlowExactLookupIndex,
    condition_offset: ConditionNodeId,
) -> Option<SalsaFlowConditionSummary> {
    find_bucket_indices(&index.by_condition_offset, &condition_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.conditions[entry].clone())
}

pub fn find_flow_branch_in_index(
    index: &SalsaFlowExactLookupIndex,
    branch_offset: TextSize,
) -> Option<SalsaFlowBranchSummary> {
    find_bucket_indices(&index.by_branch_offset, &branch_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.branches[entry].clone())
}

pub fn find_flow_loop_in_index(
    index: &SalsaFlowExactLookupIndex,
    loop_offset: TextSize,
) -> Option<SalsaFlowLoopSummary> {
    find_bucket_indices(&index.by_loop_offset, &loop_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.loops[entry].clone())
}

pub fn find_flow_return_in_index(
    index: &SalsaFlowExactLookupIndex,
    return_offset: TextSize,
) -> Option<SalsaFlowReturnSummary> {
    find_bucket_indices(&index.by_return_offset, &return_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.returns[entry].clone())
}

pub fn find_flow_break_in_index(
    index: &SalsaFlowExactLookupIndex,
    break_offset: TextSize,
) -> Option<SalsaFlowBreakSummary> {
    find_bucket_indices(&index.by_break_offset, &break_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.breaks[entry].clone())
}

pub fn find_flow_continue_in_index(
    index: &SalsaFlowExactLookupIndex,
    continue_offset: TextSize,
) -> Option<SalsaFlowContinueSummary> {
    find_bucket_indices(&index.by_continue_offset, &continue_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.continues[entry].clone())
}

pub fn find_flow_goto_in_index(
    index: &SalsaFlowExactLookupIndex,
    goto_offset: TextSize,
) -> Option<SalsaFlowGotoSummary> {
    find_bucket_indices(&index.by_goto_offset, &goto_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.gotos[entry].clone())
}

pub fn find_flow_label_in_index(
    index: &SalsaFlowExactLookupIndex,
    label_offset: TextSize,
) -> Option<SalsaFlowLabelSummary> {
    find_bucket_indices(&index.by_label_offset, &label_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.labels[entry].clone())
}
...
pub fn build_flow_query_summary(flow: &SalsaFlowSummary) -> SalsaFlowQuerySummary {
    ...
    let continue_links: Vec<SalsaFlowContinueLinkSummary> = flow
        .continues
        .iter()
        .map(|continue_summary| {
            let enclosing_loop_offset = continue_summary
                .block_offset
                .and_then(|block_offset| index.enclosing_loop_offset_of(block_offset));
            let continue_target_block_offset = enclosing_loop_offset
                .and_then(|loop_offset| index.find_loop(loop_offset))
                .and_then(|loop_summary| loop_summary.block_offset);

            SalsaFlowContinueLinkSummary {
                continue_offset: continue_summary.syntax_offset,
                block_offset: continue_summary.block_offset,
                enclosing_loop_offset,
                continue_target_block_offset,
            }
        })
        .collect();
    ...
    terminal_edges.extend(continue_links.iter().map(|link| {
        SalsaFlowTerminalEdgeSummary {
            kind: SalsaFlowTerminalEdgeKindSummary::Continue,
            syntax_offset: link.continue_offset,
            source_block_offset: link.block_offset,
            target_block_offset: link.continue_target_block_offset,
            target_label_offset: None,
            flow_region_root_offset: link
                .block_offset
                .and_then(|block_offset| index.flow_region_root_offset_of(block_offset)),
    ...
    let break_links_by_offset = break_links
        ...
        .collect::<HashMap<_, _>>();
    let continue_links_by_offset = continue_links
        .iter()
        .map(|link| (link.continue_offset, link))
        .collect::<HashMap<_, _>>();
    ...
    for statement in &flow.statements {
        ...
        match statement.kind {
            SalsaFlowStatementKindSummary::Linear | SalsaFlowStatementKindSummary::TagCast => {}
            ...
            SalsaFlowStatementKindSummary::Continue => {
                edges.push(SalsaFlowEdgeSummary {
                    kind: SalsaFlowEdgeKindSummary::StatementToTerminal,
                    from: statement_node,
                    to: SalsaFlowNodeRefSummary::Continue(statement.syntax_offset),
                });
                if let Some(link) = continue_links_by_offset.get(&statement.syntax_offset)
                    && let Some(target_block_offset) = link.continue_target_block_offset
                {
                    edges.push(SalsaFlowEdgeSummary {
                        kind: SalsaFlowEdgeKindSummary::TerminalToTarget,
                        from: SalsaFlowNodeRefSummary::Continue(statement.syntax_offset),
                        to: SalsaFlowNodeRefSummary::Block(target_block_offset),
                    });
                }
                edges.push(SalsaFlowEdgeSummary {
...
fn statement_falls_through(statement: &SalsaFlowStatementSummary) -> bool {
    matches!(
        ...
        SalsaFlowStatementKindSummary::Linear
            | SalsaFlowStatementKindSummary::Do
            | SalsaFlowStatementKindSummary::TagCast
    )
}
...
struct FlowQueryIndex<'a> {
    ...
    conditions: HashMap<ConditionNodeId, &'a SalsaFlowConditionSummary>,
...
impl<'a> FlowQueryIndex<'a> {
    ...
    fn find_condition(
        &self,
        condition_offset: ConditionNodeId,
    ) -> Option<&'a SalsaFlowConditionSummary> {
...
pub fn build_for_range_iter_query_index(
    ...
    let loops: Vec<_> = flow
        ...
        .collect();
    let by_loop_offset = build_lookup_buckets(
        loops
            .iter()
            .enumerate()
            .map(|(i, s)| (s.loop_offset, i))
            .collect(),
    );
    SalsaForRangeIterQueryIndex {
        loops,
