...
use crate::{
    ...
    SalsaDocTypeLoweredIndex, SalsaDocTypeLoweredKind, SalsaDocTypeLoweredNode,
    SalsaDocTypeNodeKey, SalsaDocTypeRef, SalsaGlobalRootSummary, SalsaLexicalUseIndex,
    SalsaLocalAssignmentQueryIndex, SalsaMemberRootSummary, SalsaMemberTargetSummary,
    SalsaNameUseResolutionSummary, SalsaPropertyIndexSummary, SalsaSemanticResolveStateSummary,
...
pub struct SalsaSignatureGenericParamLookupKey {
    pub owner_offset: TextSize,
    pub name: SmolStr,
}

#[derive(Debug, Clone, PartialEq, Eq, salsa::Update)]
pub struct SalsaSignatureGenericParamLookupSummary {
    pub signature_offset: TextSize,
    pub tpl_id: u32,
    pub param: SalsaSignatureGenericParamExplainSummary,
}
...
pub struct SalsaSignatureExplainIndex {
    ...
    pub generic_params: Vec<SalsaSignatureGenericParamLookupSummary>,
    ...
    by_generic_param_key: Vec<SalsaLookupBucket<SalsaSignatureGenericParamLookupKey>>,
}
...
pub fn build_signature_explain_index(
    ...
    let mut generic_param_key_entries = Vec::new();
    for signature in &signatures {
        for owner in &signature.doc_owners {
            for (tpl_id, param) in signature
                .generics
                .iter()
                .flat_map(|generic| generic.params.iter())
                .enumerate()
            {
                let generic_param = SalsaSignatureGenericParamLookupSummary {
                    signature_offset: signature.signature.syntax_offset,
                    tpl_id: tpl_id as u32,
                    param: param.clone(),
                };
                let generic_param_index = generic_params.len();
                generic_params.push(generic_param);
                generic_param_key_entries.push((
                    SalsaSignatureGenericParamLookupKey {
                        owner_offset: owner.owner_offset,
    ...
    SalsaSignatureExplainIndex {
        ...
        by_signature_name,
        by_generic_param_key: build_lookup_buckets(generic_param_key_entries),
    }
}
...
pub fn find_signature_generic_param_by_owner_in_index(
    index: &SalsaSignatureExplainIndex,
    owner_offset: TextSize,
    name: &str,
) -> Option<SalsaSignatureGenericParamLookupSummary> {
    let key = SalsaSignatureGenericParamLookupKey {
        owner_offset,
        name: SmolStr::new(name),
    };
    find_bucket_indices(&index.by_generic_param_key, &key)
        .and_then(|indices| indices.first().copied())
        .map(|generic_param_index| index.generic_params[generic_param_index].clone())
}
...
pub fn build_signature_return_query_index(
    ...
    let signatures: Vec<_> = signatures_index
        .signatures
        .iter()
        .map(|signature| {
        ...
        .collect();
    let by_signature_offset = build_lookup_buckets(
        signatures
            .iter()
            .enumerate()
            .map(|(i, s)| (s.signature_offset, i))
            .collect(),
    );
    SalsaSignatureReturnQueryIndex {
        signatures,
...
pub fn find_signature_return_query_at(
    ...
    find_bucket_indices(&index.by_signature_offset, &signature_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| index.signatures[entry].clone())
}
...
fn signature_type_is_any_like(doc_type: &SalsaSignatureTypeExplainSummary) -> bool {
    match doc_type.lowered.as_ref().map(|lowered| &lowered.kind) {
        Some(SalsaDocTypeLoweredKind::Unknown) => true,
        Some(SalsaDocTypeLoweredKind::Name { name }) => {
