...
pub fn find_name_use_at(
    ...
    if let Some(exact) = find_bucket_indices(&use_sites.by_name_offset, &syntax_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| use_sites.names[entry].clone())
    {
        return Some(exact);
    }
...
pub fn find_member_use_at(
    ...
    if let Some(exact) = find_bucket_indices(&use_sites.by_member_offset, &syntax_offset)
        .and_then(|indices| indices.first().copied())
        .map(|entry| use_sites.members[entry].clone())
    {
        return Some(exact);
    }
