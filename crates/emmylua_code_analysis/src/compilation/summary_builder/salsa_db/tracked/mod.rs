use crate::{
    DiagnosticCode, FileId, SalsaDocDiagnosticActionKindSummary, SalsaForRangeIterQueryIndex,
    SalsaForRangeIterQuerySummary, SalsaSemanticTargetSummary, build_module_summary_with_index,
};
use crate::{
...
use super::{
    super::{
        ...
        query::{
            SalsaCallExplainSummary, SalsaDeclQueryIndex, SalsaDeclTypeInfoSummary,
            SalsaDeclTypeQueryIndex, SalsaDocOwnerResolveIndex, SalsaDocOwnerResolveSummary,
            SalsaDocTagPropertySummary, SalsaDocTagQueryIndex, SalsaDocTypeLoweredIndex,
            SalsaDocTypeLoweredNode, SalsaDocTypeResolvedIndex, SalsaDocTypeResolvedSummary,
            SalsaFlowExactLookupIndex, SalsaGlobalTypeInfoSummary, SalsaGlobalTypeQueryIndex,
            SalsaLexicalUseIndex, SalsaLexicalUseSummary, SalsaLocalAssignmentQueryIndex,
            SalsaMemberQueryIndex, SalsaMemberTypeInfoSummary, SalsaMemberTypeQueryIndex,
            ...
            SalsaSignatureGenericParamLookupSummary, SalsaSignatureReturnQueryIndex,
            SalsaSignatureReturnQuerySummary, SalsaSingleFileSemanticSummary,
            SalsaTableShapeQueryIndex, build_branch_graph_summary, build_condition_graph_summary,
            build_decl_query_index, build_decl_type_query_index, build_decl_value_shell,
            build_doc_owner_resolve_index, build_doc_tag_query_index,
            build_flow_exact_lookup_index, build_flow_query_summary,
            ...
            build_lowered_doc_type_index, build_member_query_index, build_member_type_query_index,
            build_member_value_shell, build_module_resolve_index, build_property_query_index,
            build_resolved_doc_type_index, build_semantic_decl_summary,
            build_semantic_graph_query_index, build_semantic_graph_scc_index,
            build_semantic_graph_summary, build_semantic_member_summary,
            build_semantic_solver_execution, build_semantic_solver_worklist,
            build_semantic_target_query_index,
            ...
            build_table_shape_query_index, build_terminal_graph_summary, can_reach_node,
            collect_active_type_narrows, collect_call_references_for_member_in_index,
            ...
            collect_doc_owner_resolves_for_signature, collect_doc_tags_for_kind_in_index,
            collect_doc_tags_for_owner_in_index, collect_global_name_references_in_index,
            collect_incoming_edges, collect_incoming_semantic_graph_edges,
            collect_member_references_by_role_in_index, collect_member_references_in_index,
            collect_name_references_by_role_in_index, collect_outgoing_edges,
            collect_outgoing_semantic_graph_edges, collect_predecessor_nodes,
            collect_properties_for_decl_and_key_in_index, collect_properties_for_decl_in_index,
            collect_properties_for_key_in_index, collect_properties_for_member_and_key_in_index,
            collect_properties_for_member_in_index, collect_properties_for_source_in_index,
            collect_properties_for_type_and_key_in_index, collect_properties_for_type_in_index,
            collect_reachable_nodes, collect_resolved_doc_tag_diagnostics_for_property,
            ...
            find_call_use_in_index, find_decl_by_syntax_id_in_index, find_decl_type_info,
            find_doc_owner_resolve_at, find_doc_tag_at_in_index, find_doc_tag_property_in_index,
            find_flow_block_in_index, find_flow_branch_in_index, find_flow_break_in_index,
            find_flow_condition_in_index, find_flow_continue_in_index, find_flow_goto_in_index,
            find_flow_label_in_index, find_flow_loop_in_index, find_flow_return_in_index,
            find_for_range_iter_query_at, find_global_name_type_info, find_global_type_info,
            find_lexical_use_at, find_lowered_doc_type_at, find_lowered_doc_type_by_key,
            find_member_by_syntax_id_in_index, find_member_type_at_program_point,
            find_member_type_info, find_member_use_at, find_member_use_by_syntax_id,
            find_member_use_type_info, find_module_export, find_module_export_target,
            find_module_exported_global_function, find_module_exported_global_variable,
            find_name_type_at_program_point, find_name_type_info, find_name_use_at,
            find_name_use_by_syntax_id, find_next_ready_semantic_solver_execution_task,
            find_property_at_in_index, find_property_by_value_expr_syntax_id_in_index,
            find_resolved_doc_type_at, find_resolved_doc_type_by_key, find_semantic_decl_in_index,
            ...
            find_signature_explain_at, find_signature_generic_param_by_owner_in_index,
            find_signature_return_query_at, find_table_shape_at_in_index,
            find_table_shape_by_syntax_id_in_index, for_range_iter_resolve_state_from_semantic,
            module_export_resolve_state_from_semantic, module_exports_closure, module_exports_decl,
            module_exports_global_function, module_exports_global_variable, module_exports_member,
            module_exports_table, semantic_solver_execution_is_complete,
            signature_return_resolve_state_from_semantic,
    ...
    inputs::{SummaryConfigInput, SummarySourceFileInput, config_input, file_input, parse_chunk},
};
...
#[salsa::tracked]
pub(crate) fn tracked_parse_syntax_tree(
    db: &dyn SummaryDb,
...
fn tracked_file_decl_analysis(
    ...
    let parsed = parse_chunk(db, file, config);
...
#[salsa::tracked]
fn tracked_file_flow_exact_lookup_index(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
) -> Arc<SalsaFlowExactLookupIndex> {
    let flow = tracked_file_flow_summary(db, file, config);
    Arc::new(build_flow_exact_lookup_index(flow.as_ref()))
}
...
fn tracked_file_flow_block(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_block_in_index(index.as_ref(), block_offset)
}

#[salsa::tracked]
fn tracked_file_flow_condition(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    condition_node_offset: ConditionNodeId,
) -> Option<SalsaFlowConditionSummary> {
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_condition_in_index(index.as_ref(), condition_node_offset)
}
...
fn tracked_file_flow_branch(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_branch_in_index(index.as_ref(), branch_offset)
}
...
fn tracked_file_flow_loop(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_loop_in_index(index.as_ref(), loop_offset)
}
...
fn tracked_file_flow_return(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_return_in_index(index.as_ref(), return_offset)
}
...
fn tracked_file_flow_break(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_break_in_index(index.as_ref(), break_offset)
}

#[salsa::tracked]
fn tracked_file_flow_continue(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    continue_offset: TextSize,
) -> Option<SalsaFlowContinueSummary> {
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_continue_in_index(index.as_ref(), continue_offset)
}
...
fn tracked_file_flow_goto(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_goto_in_index(index.as_ref(), goto_offset)
}
...
fn tracked_file_flow_label(
    ...
    let index = tracked_file_flow_exact_lookup_index(db, file, config);
    find_flow_label_in_index(index.as_ref(), label_offset)
}
...
#[salsa::tracked]
fn tracked_file_decl_query_index(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
) -> Arc<SalsaDeclQueryIndex> {
    let decl_tree = tracked_file_decl_tree_summary(db, file, config);
    Arc::new(build_decl_query_index(decl_tree.as_ref()))
}

#[salsa::tracked]
fn tracked_file_member_query_index(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
) -> Arc<SalsaMemberQueryIndex> {
    let members = tracked_file_member_summary(db, file, config);
    Arc::new(build_member_query_index(members.as_ref()))
}
...
fn tracked_file_decl_by_syntax_id(
    ...
    let index = tracked_file_decl_query_index(db, file, config);
    find_decl_by_syntax_id_in_index(index.as_ref(), syntax_id)
}
...
fn tracked_file_member_by_syntax_id(
    ...
    let index = tracked_file_member_query_index(db, file, config);
    find_member_by_syntax_id_in_index(index.as_ref(), syntax_id)
}
...
#[salsa::tracked]
fn tracked_file_table_shape_query_index(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
) -> Arc<SalsaTableShapeQueryIndex> {
    let table_shapes = tracked_file_table_shape_summary(db, file, config);
    Arc::new(build_table_shape_query_index(table_shapes.as_ref()))
}

#[salsa::tracked]
fn tracked_file_table_shape_at(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    syntax_offset: TextSize,
) -> Option<SalsaTableShapeSummary> {
    let index = tracked_file_table_shape_query_index(db, file, config);
    find_table_shape_at_in_index(index.as_ref(), syntax_offset)
}

#[salsa::tracked]
fn tracked_file_table_shape_by_syntax_id(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    syntax_id: SalsaSyntaxIdSummary,
    ...
    let index = tracked_file_table_shape_query_index(db, file, config);
    find_table_shape_by_syntax_id_in_index(index.as_ref(), syntax_id)
}
...
/// 缓存的文件诊断状态（聚合 @diagnostic tag 解析结果）。
/// 内部依赖 salsa-tracked 子查询，结果本身不需要额外 #[salsa::tracked]。
fn tracked_file_diagnostic_status(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
) -> Option<Arc<SalsaFileDiagnosticStatus>> {
    let summary = file_doc_summary(db, file_id)?;
    let mut disabled_codes = Vec::new();
    let mut enabled_codes = Vec::new();
    let mut actions = Vec::new();

    for tag in &summary.tags {
        if tag.kind != SalsaDocTagKindSummary::Diagnostic {
            continue;
        }
        let diags = file_resolved_doc_tag_diagnostics(db, file_id, tag.owner.clone())?;
        for a in &diags {
            let code = a.code.unwrap_or(DiagnosticCode::None);
            match a.kind {
                SalsaDocDiagnosticActionKindSummary::Disable => {
                    disabled_codes.push(code.clone());
                }
                SalsaDocDiagnosticActionKindSummary::Enable => {
                    enabled_codes.push(code.clone());
                }
                _ => actions.push(SalsaDiagAction {
                    range: a.range,
                    code: Some(code),
                    is_disable: true,
                }),
    ...
    Some(Arc::new(SalsaFileDiagnosticStatus {
        disabled_codes,
        enabled_codes,
        actions,
    }))
...
pub(crate) fn file_table_shape_at(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    syntax_offset: TextSize,
) -> Option<SalsaTableShapeSummary> {
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_table_shape_at(db, file, config, syntax_offset)
}

pub(crate) fn file_table_shape_by_syntax_id(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    syntax_id: SalsaSyntaxIdSummary,
) -> Option<SalsaTableShapeSummary> {
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_table_shape_by_syntax_id(db, file, config, syntax_id)
}
...
pub(crate) fn file_diagnostic_status(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
) -> Option<Arc<SalsaFileDiagnosticStatus>> {
    tracked_file_diagnostic_status(db, file_id)
}
...
pub(crate) fn file_flow_continue(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    continue_offset: TextSize,
) -> Option<SalsaFlowContinueSummary> {
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_flow_continue(db, file, config, continue_offset)
}
...
pub(crate) fn file_flow_condition(
    ...
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_flow_condition(db, file, config, condition_node_offset)
}
...
pub(crate) fn file_flow_continue_graph(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    continue_offset: TextSize,
) -> Option<SalsaFlowTerminalGraphSummary> {
    file_flow_continue(db, file_id, continue_offset)?;
    let query = file_flow_query_summary(db, file_id)?;
    Some(build_terminal_graph_summary(
        query.as_ref(),
        SalsaFlowNodeRefSummary::Continue(continue_offset),
    ))
}
