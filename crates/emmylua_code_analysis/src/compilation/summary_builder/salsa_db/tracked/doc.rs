...
use crate::{
    SalsaResolvedDocDiagnosticActionSummary,
    compilation::summary_builder::query::{
        SalsaDocTypeDefQueryIndex, build_doc_type_def_query_index, find_type_def_by_name_in_index,
    },
};

/// LineIndex 的 salsa 兼容包装。
#[derive(Debug, Clone, PartialEq, Eq, Hash, salsa::Update)]
pub struct CachedLineIndex(pub LineIndex);
...
#[salsa::tracked]
pub(crate) fn tracked_file_doc_type_def_query_index(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
) -> Arc<SalsaDocTypeDefQueryIndex> {
    let doc = tracked_file_doc_summary(db, file, config);
    Arc::new(build_doc_type_def_query_index(doc.as_ref()))
}

#[salsa::tracked]
pub(crate) fn tracked_file_doc_type_def_by_name(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    name: SmolStr,
) -> Option<SalsaDocTypeDefSummary> {
    let index = tracked_file_doc_type_def_query_index(db, file, config);
    find_type_def_by_name_in_index(index.as_ref(), &name)
}
...
pub(crate) fn tracked_file_resolved_doc_tag_diagnostics(
    ...
    let chunk = parse_chunk(db, file.file_id(db), file.text(db), &config.config(db));
    let line_index = tracked_line_index(db, file);
...
#[salsa::tracked]
pub(crate) fn tracked_file_signature_generic_param_by_owner(
    db: &dyn SummaryDb,
    file: SummarySourceFileInput,
    config: SummaryConfigInput,
    ...
    let index = tracked_file_signature_explain_index(db, file, config);
    find_signature_generic_param_by_owner_in_index(index.as_ref(), owner_offset, &name)
}
...
pub(crate) fn file_doc_type_def_query_index(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
) -> Option<Arc<SalsaDocTypeDefQueryIndex>> {
    let (file, config) = file_and_config(db, file_id)?;
    Some(tracked_file_doc_type_def_query_index(db, file, config))
}

pub(crate) fn file_doc_type_def_by_name(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    name: SmolStr,
) -> Option<SalsaDocTypeDefSummary> {
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_doc_type_def_by_name(db, file, config, name)
}
...
pub(crate) fn file_signature_generic_param_by_owner(
    db: &SalsaSummaryDatabase,
    file_id: FileId,
    owner_offset: TextSize,
    name: SmolStr,
    ...
    let (file, config) = file_and_config(db, file_id)?;
    tracked_file_signature_generic_param_by_owner(db, file, config, owner_offset, name)
}
