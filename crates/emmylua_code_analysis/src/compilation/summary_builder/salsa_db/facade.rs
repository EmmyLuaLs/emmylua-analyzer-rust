use crate::{
    ConditionNodeId, FileId, SalsaDeclSummary, SalsaDocTypeNodeKey, SalsaFileDiagnosticStatus,
    SalsaFlowBreakSummary, SalsaFlowNodeRefSummary, SalsaGlobalFunctionSummary,
    SalsaGlobalVariableSummary, SalsaModuleExportQuerySummary, SalsaModuleResolveIndex,
...
use crate::semantic_model::offset_types::SyntaxPosition;
use crate::{
    ...
    SalsaFlowConditionGraphSummary, SalsaFlowConditionSummary, SalsaFlowContinueSummary,
    SalsaFlowEdgeSummary, SalsaFlowGotoSummary, SalsaFlowLabelSummary, SalsaFlowLoopGraphSummary,
    SalsaFlowLoopSummary, SalsaFlowQuerySummary, SalsaFlowReturnSummary, SalsaFlowSummary,
    SalsaFlowTerminalGraphSummary, SalsaForRangeIterQueryIndex, SalsaForRangeIterQuerySummary,
    SalsaGlobalSummary, SalsaGlobalTypeInfoSummary, SalsaGlobalTypeQueryIndex,
    SalsaLexicalUseIndex, SalsaLexicalUseSummary, SalsaMemberIndexSummary, SalsaMemberSummary,
    SalsaMemberTargetId, SalsaMemberTypeInfoSummary, SalsaMemberTypeQueryIndex,
    SalsaMemberUseSummary, SalsaModuleExportSemanticSummary, SalsaModuleExportSummary,
    SalsaModuleSummary, SalsaNameTypeInfoSummary, SalsaNameUseSummary,
    SalsaProgramPointMemberTypeInfoSummary, SalsaProgramPointTypeInfoSummary,
    SalsaPropertyIndexSummary, SalsaPropertyKeySummary, SalsaPropertyOwnerSummary,
    ...
    SalsaTableShapeSummary, SalsaUseSiteIndexSummary, TypeDefEntry, WorkspaceMemberIndex,
...
impl<'db> SalsaSummaryDocQueries<'db> {
    ...
    pub fn type_def_by_name(&self, file_id: FileId, name: &str) -> Option<SalsaDocTypeDefSummary> {
    ...
    /// 缓存的文件诊断状态（含行级范围匹配的 @diagnostic 动作）。
    pub fn diagnostic_status(&self, file_id: FileId) -> Option<Arc<SalsaFileDiagnosticStatus>> {
        tracked::file_diagnostic_status(self.db, file_id)
    }
}
...
impl<'db> SalsaSummaryFlowQueries<'db> {
    ...
    pub fn continue_at(
        ...
        tracked::file_flow_continue(self.db, file_id, continue_offset)
    }
    ...
    pub fn continue_graph(
        &self,
        file_id: FileId,
        continue_offset: TextSize,
    ) -> Option<SalsaFlowTerminalGraphSummary> {
        tracked::file_flow_continue_graph(self.db, file_id, continue_offset)
    }
...
impl<'db> SalsaSummarySemanticQueries<'db> {
    ...
    pub fn member_index(&self) -> Option<Arc<WorkspaceMemberIndex>> {
        ...
        for file_id in &file_ids {
            ...
            for prop in &index.properties {
                ...
                if let Some((_, existing)) = by_type.iter_mut().find(|(n, _)| n == &type_name) {
                    if !existing
                        .iter()
                        .any(|e| e.key == entry.key && e.file_id == entry.file_id)
                    {
                        existing.push(entry);
                    }
                } else {
        ...
        if by_type.is_empty() {
            ...
            Some(Arc::new(WorkspaceMemberIndex { by_type }))
        }
    }
    ...
    pub fn type_index(&self) -> Option<Arc<WorkspaceTypeIndex>> {
        ...
        for file_id in &file_ids {
            let Some(doc) = self.db.doc().summary(*file_id) else {
                continue;
            };
            ...
            for td in &doc.type_defs {
                ...
                if let Some((_, existing)) = by_name.iter_mut().find(|(n, _)| n == &td.name) {
                    if !existing.iter().any(|e| {
                        e.file_id == entry.file_id && e.syntax_offset == entry.syntax_offset
                    }) {
                        existing.push(entry);
                    }
                } else {
        ...
        if by_name.is_empty() {
            ...
            Some(Arc::new(WorkspaceTypeIndex { by_name, by_file }))
        }
    }
}
...
impl<'db> SalsaSummaryTypeQueries<'db> {
    ...
    /// Look up member type at a specific program point.
    /// Both offsets are syntax positions.
    pub fn member_at(
        &self,
        file_id: FileId,
        syntax_offset: impl Into<SyntaxPosition>,
        program_point_offset: impl Into<SyntaxPosition>,
    ) -> Option<SalsaProgramPointMemberTypeInfoSummary> {
        tracked::file_member_type_at_program_point(
            self.db,
            file_id,
            syntax_offset.into().into(),
            program_point_offset.into().into(),
        )
    }
    ...
    /// Look up name type at a specific program point.
    /// Both offsets are syntax positions.
    pub fn name_at(
        &self,
        file_id: FileId,
