...
impl SalsaSummaryDatabase {
    ...
    /// The syntax tree is parsed lazily via `tracked_parse_syntax_tree` (salsa-tracked).
    pub fn set_file(
        &mut self,
        file_id: FileId,
        path: Option<PathBuf>,
    ...
    pub fn get_syntax_tree(&self, file_id: FileId) -> Option<LuaSyntaxTree> {
        let (file, config) = tracked::file_and_config(self, file_id)?;
        let tree = tracked::tracked_parse_syntax_tree(self, file, config);
        Some(LuaSyntaxTree::clone(&tree))
    }
