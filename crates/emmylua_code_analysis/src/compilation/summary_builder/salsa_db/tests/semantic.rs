...
#[test]
fn test_summary_builder_signature_generic_param_lookup_preserves_default_type() {
    let mut compilation = setup_compilation();
    set_test_file(
        &mut compilation,
        30,
        "C:/ws/signature_generic_default.lua",
        r#"
            ---@generic T = string
            ---@return T
    ...
    let signature = signatures
        .signatures
        .iter()
        .find(|signature| signature.name.as_deref() == Some("use_default"))
        .expect("use_default signature");
    let generic_param = doc
        .signature()
        .generic_param(FileId::new(30), signature.owner_offset, "T")
        .expect("signature generic param");

    let default_type = generic_param
        .param
        .default_type
        .as_ref()
        .expect("default type explain");
    let lowered = default_type.lowered.as_ref().expect("lowered default type");

    assert!(matches!(
        lowered.kind,
        SalsaDocTypeLoweredKind::Name { ref name } if name == "string"
    ));
}
...
local out = Box:run(1)"#;
    ...
    let generic_owner_offset = semantic_signature_summary
        .explain
        .doc_owners
        .first()
        .map(|owner| owner.owner_offset)
        .expect("generic owner offset");
    let generic_param = doc
        .signature()
        .generic_param(FileId::new(20), generic_owner_offset, "T")
        .expect("signature generic param");
    assert_eq!(generic_param.signature_offset, signature_offset);
    assert_eq!(generic_param.tpl_id, 0);
    assert!(matches!(
    ...
    assert!(matches!(
        generic_param.param.bound_type,
        Some(SalsaSignatureTypeExplainSummary {
            lowered: Some(SalsaDocTypeLoweredNode {
                kind: SalsaDocTypeLoweredKind::Name { ref name },
                ..
