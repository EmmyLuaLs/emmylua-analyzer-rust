...
fn test_summary_builder_flow_structures() {
  ...
  return items
    ...
    let return_offset = flow.returns[0].syntax_offset;
    ...
    assert!(matches!(
        flow_api.return_at(FileId::new(11), return_offset),
        Some(crate::SalsaFlowReturnSummary { .. })
    ));
    ...
    assert!(matches!(
        flow_api.condition(FileId::new(11), while_condition_offset),
        Some(crate::SalsaFlowConditionSummary {
            kind: crate::SalsaFlowConditionKindSummary::Expr,
            ..
        })
    ));
...
fn test_summary_builder_flow_continue_statement() {
    ...
    let source = r#"local function run(items)
  for i = 1, 10 do
    if i == 5 then
      continue
    end
    ...
    set_test_file(&mut db, 20, "C:/ws/continue_flow.lua", source);

    let flow_api = db.flow();
    let flow = flow_api.summary(FileId::new(20)).expect("flow summary");
    let flow_query = flow_api.query(FileId::new(20)).expect("flow query summary");

    assert_eq!(flow.break_count, 0);
    assert_eq!(
        flow.continue_count, 1,
        "expected one continue statement in the flow"
    );

    let continue_link = flow_query
        .continue_links
        .first()
        .expect("expected a continue link");
    assert!(
        continue_link.enclosing_loop_offset.is_some(),
        "continue should have an enclosing loop offset"
    );

    let terminal_edge = flow_query
        .terminal_edges
        .iter()
        .find(|edge| matches!(edge.kind, crate::SalsaFlowTerminalEdgeKindSummary::Continue))
        .expect("expected a continue terminal edge");
    assert_eq!(terminal_edge.syntax_offset, continue_link.continue_offset);

    let continue_offset = flow
        .continues
        .first()
        .expect("flow continues")
        .syntax_offset;
    let continue_stat = flow_api.continue_at(FileId::new(20), continue_offset);
    assert!(continue_stat.is_some());

    let continue_graph = flow_api
        .continue_graph(FileId::new(20), continue_offset)
        .expect("continue graph");
    assert!(continue_graph.unreachable_node.is_some());
    assert!(!continue_graph.target_nodes.is_empty());
}
