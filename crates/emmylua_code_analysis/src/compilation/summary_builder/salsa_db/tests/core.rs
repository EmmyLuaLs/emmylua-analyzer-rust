use super::*;
use emmylua_parser::{LuaIndexExpr, LuaLocalName};
...
#[test]
fn test_summary_builder_decl_and_member_lookup_by_syntax_id() {
    let source = r#"local box = {}
box.value = 1"#;
    let mut compilation = setup_compilation();
    set_test_file(&mut compilation, 2, "C:/ws/lookup.lua", source);
    ...
    let file = compilation.file();
    let decl = file
        .decl_by_syntax_id(FileId::new(2), local_name.get_syntax_id().into())
        .expect("decl by syntax id");
    let member = file
        .member_by_syntax_id(FileId::new(2), member_expr.get_syntax_id().into())
        .expect("member by syntax id");

    assert_eq!(decl.name, "box");
    assert_eq!(member.target.member_name.as_str(), "value");
}
