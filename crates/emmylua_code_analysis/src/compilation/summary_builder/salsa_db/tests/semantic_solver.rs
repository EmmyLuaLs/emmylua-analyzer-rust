...
"#;
    ...
    let next_execution = complete_task(&execution, mid_component.component_id);
    let next_leaf_task =
...
"#;
    ...
    let resolved_ready_execution = resolved_component.predecessor_component_ids.iter().fold(
        ...
        |execution_state, predecessor_component_id| {
            complete_task(&execution_state, *predecessor_component_id).into()
        },
    );
