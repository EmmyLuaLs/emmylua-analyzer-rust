use crate::{SalsaSignatureReturnExprKindSummary, SalsaSignatureReturnResolveStateSummary};
...
function test()
  return value
    ...
    assert!(matches!(
        ...
        SalsaSignatureReturnExprKindSummary::Name
    ));
    assert!(
        summary.values[0]
            .name_type
...
local function ret_closure()
  return function()
    ...
    for (name, expected_kind) in [
        ("ret_literal", SalsaSignatureReturnExprKindSummary::Literal),
        ("ret_table", SalsaSignatureReturnExprKindSummary::Table),
        ("ret_closure", SalsaSignatureReturnExprKindSummary::Closure),
    ] {
        let signature = signatures
            .signatures
            .iter()
  ...
  return holder.first, holder.second
    ...
    assert!(
        summary
            .values
            .iter()
            .all(|value| matches!(value.kind, SalsaSignatureReturnExprKindSummary::Member))
    );
    assert!(!summary.values[0].doc_return_type_offsets.is_empty());
    assert!(!summary.values[1].doc_return_type_offsets.is_empty());
    assert_ne!(
