use crate::{SalsaSequenceShapeKindSummary, SalsaTableShapeKindSummary};

use super::*;
use emmylua_parser::{LuaAstNode, LuaParser, LuaTableExpr, ParserConfig};

#[test]
fn test_summary_builder_table_shape_classifies_empty_object_sequence_and_mixed_tables() {
    let mut compilation = setup_compilation();
    let source = r#"local empty = {}
local object = { value = 1, name = "x" }
local array = { 1, 2, 3 }
local tuple = { 1, "x" }
local mixed = { value = 1, 2 }"#;
    set_test_file(&mut compilation, 155, "C:/ws/table_shape.lua", source);

    let table_shapes = compilation
        .file()
        .table_shapes(FileId::new(155))
        .expect("table shape summary");

    assert_eq!(table_shapes.tables.len(), 5);

    assert_eq!(
        table_shapes.tables[0].kind,
        SalsaTableShapeKindSummary::Empty
    );
    assert_eq!(
        table_shapes.tables[1].kind,
        SalsaTableShapeKindSummary::ObjectLike
    );
    assert_eq!(
        table_shapes.tables[2].kind,
        SalsaTableShapeKindSummary::SequenceLike
    );
    assert_eq!(
        table_shapes.tables[2].sequence_kind,
        SalsaSequenceShapeKindSummary::ArrayLike
    );
    assert_eq!(table_shapes.tables[2].sequence_len, 3);
    assert_eq!(
        table_shapes.tables[3].kind,
        SalsaTableShapeKindSummary::SequenceLike
    );
    assert_eq!(
        table_shapes.tables[3].sequence_kind,
        SalsaSequenceShapeKindSummary::TupleLike
    );
    assert_eq!(table_shapes.tables[3].sequence_len, 2);
    assert_eq!(
        table_shapes.tables[4].kind,
        SalsaTableShapeKindSummary::Mixed
    );
}

#[test]
fn test_summary_builder_table_shape_supports_direct_lookup_and_range_projection() {
    let mut compilation = setup_compilation();
    let source = r#"local exported = { answer = 42 }"#;
    set_test_file(
        &mut compilation,
        156,
        "C:/ws/table_shape_lookup.lua",
        source,
    );

    let tree = LuaParser::parse(source, ParserConfig::default());
    let table_expr = tree
        .get_chunk_node()
        .descendants::<LuaTableExpr>()
        .next()
        .expect("table expr");

    let file = compilation.file();
    let by_offset = file
        .table_shape_at(FileId::new(156), table_expr.get_position())
        .expect("table shape by offset");
    let by_syntax_id = file
        .table_shape_by_syntax_id(FileId::new(156), table_expr.get_syntax_id().into())
        .expect("table shape by syntax id");

    assert_eq!(by_offset, by_syntax_id);
    assert_eq!(by_offset.syntax_id.text_range(), table_expr.get_range());
}
