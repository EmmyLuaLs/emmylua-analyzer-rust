...
use emmylua_parser::{
    LuaChunk, LuaFeatures, LuaFeaturesSet, LuaLanguageLevel, ParserConfig, SpecialFunction,
};
use rowan::NodeCache;
...
pub(crate) fn parse_chunk(
    ...
    let tree = super::tracked::tracked_parse_syntax_tree(db, file, config);
    InFiled::new(file.file_id(db), tree.get_chunk_node())
}
