use crate::{
    DbIndex, FileId, InFiled, LuaDeclId, LuaSignatureId, LuaType, ModuleVisibility,
    SalsaDocVisibilityKindSummary, SalsaExportTargetSummary, SalsaModuleExportQuerySummary,
    SalsaModuleExportSummary, SalsaSemanticTargetSummary, WorkspaceId,
};
...
pub fn resolve_projected_module_export_type(db: &DbIndex, file_id: FileId) -> Option<LuaType> {
    ...
    match module_info.semantic_target {
        ...
        Some(SalsaSemanticTargetSummary::Signature(signature_offset)) => Some(LuaType::Signature(
            LuaSignatureId::from_position(file_id, signature_offset),
        )),
    ...
    match export? {
        SalsaModuleExportSummary::Table { table_offset } => {
            let table_range = db
                .get_summary_db()
                .file()
                .table_shape_at(file_id, table_offset)?
                .syntax_id
                .text_range();
            Some(LuaType::TableConst(InFiled::new(file_id, table_range)))
        }
        SalsaModuleExportSummary::Closure { signature_offset } => Some(LuaType::Signature(
            LuaSignatureId::from_position(file_id, signature_offset),
        )),
