...
pub fn remove_type(db: &DbIndex, source: LuaType, removed_type: LuaType) -> Option<LuaType> {
    ...
    let real_type = crate::semantic::type_queries::get_real_type(db, &source).unwrap_or(&source);
    ...
    match &removed_type {
        ...
        LuaType::Table => match &real_type {
            ...
            LuaType::Ref(type_decl_id) | LuaType::Def(type_decl_id) => {
                let type_decl = crate::semantic::type_queries::get_type_decl(db, type_decl_id)?;
                ...
                if type_decl.is_alias()
                    && let Some(alias_ref) =
                        crate::semantic::type_queries::get_real_type(db, real_type)
                {
                    return remove_type(db, alias_ref.clone(), removed_type);
                }

                // 需要对`userdata`进行特殊处理
                if crate::semantic::type_queries::has_userdata_super_type(db, type_decl_id) {
                    return Some(source.clone());
                }
                return None;
            }
