...
use crate::{DbIndex, LuaFunctionType, LuaType, LuaUnionType};

pub fn union_type(db: &DbIndex, source: LuaType, target: LuaType) -> LuaType {
    let match_source = crate::semantic::type_queries::get_real_type(db, &source)
        .cloned()
        .unwrap_or_else(|| source.clone());
    canonicalize_callable_union_with(db, union_type_impl(&match_source, source, target))
}

pub(crate) fn union_type_shallow(source: LuaType, target: LuaType) -> LuaType {
    let match_source = source.clone();
    canonicalize_callable_union_shallow(union_type_impl(&match_source, source, target))
}

pub(crate) fn canonicalize_callable_union_shallow(ty: LuaType) -> LuaType {
    canonicalize_callable_union_by(ty, |candidate| match candidate {
...
fn canonicalize_callable_union_with(db: &DbIndex, ty: LuaType) -> LuaType {
    canonicalize_callable_union_by(ty, |candidate| as_callable_type(db, candidate))
}

fn canonicalize_callable_union_by<F>(ty: LuaType, mut as_callable: F) -> LuaType
where
    F: FnMut(&LuaType) -> Option<Arc<LuaFunctionType>>,
{
    ...
    for member in union.into_vec() {
        let member_callable = as_callable(&member);
        if types.iter().any(|existing| {
            existing == &member
                || as_callable(existing)
                    .as_ref()
...
fn as_callable_type(db: &DbIndex, ty: &LuaType) -> Option<Arc<LuaFunctionType>> {
    match ty {
        ...
        LuaType::Signature(signature_id) => {
            crate::semantic::type_queries::get_signature(db, signature_id)
