...
impl<'a> TypeHumanizer<'a> {
    ...
    fn write_ref_type<W: Write>(&mut self, id: &LuaTypeDeclId, w: &mut W) -> fmt::Result {
        if let Some(type_decl) = crate::semantic::type_queries::get_type_decl(self.db, id) {
    ...
    fn write_def_type<W: Write>(&mut self, id: &LuaTypeDeclId, w: &mut W) -> fmt::Result {
        let type_decl = match crate::semantic::type_queries::get_type_decl(self.db, id) {
            Some(type_decl) => type_decl,
            None => return w.write_str(id.get_name()),
        };

        let full_name = type_decl.get_full_name().to_string();
        let generic = match crate::semantic::type_queries::get_generic_params(self.db, id) {
    ...
    fn write_simple_type<W: Write>(
        ...
        let members =
            match crate::semantic::type_queries::get_sorted_members(self.db, &member_owner) {
                Some(m) => m,
                None => {
        ...
        for member in members {
            ...
            let type_cache =
                crate::semantic::type_queries::get_type_cache(self.db, &member.get_id());
    ...
    fn write_generic_type<W: Write>(&mut self, generic: &LuaGenericType, w: &mut W) -> fmt::Result {
        ...
        let type_decl = match crate::semantic::type_queries::get_type_decl(self.db, &base_id) {
            Some(type_decl) => type_decl,
    ...
    fn write_table_const_detail_or_simple<W: Write>(
        ...
        let members =
            match crate::semantic::type_queries::get_sorted_members(self.db, &member_owned) {
                Some(m) => m,
    ...
    fn write_signature_type<W: Write>(
        ...
        let signature = match crate::semantic::type_queries::get_signature(self.db, signature_id) {
            Some(sig) => sig,
