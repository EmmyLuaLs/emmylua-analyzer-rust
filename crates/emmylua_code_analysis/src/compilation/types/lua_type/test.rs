...
mod tests {
    ...
    use crate::{
        GenericTpl, GenericTplId, LuaArrayType, LuaType, LuaTypeDeclId, TypeVisitTrait,
        VariadicType,
    };
    ...
    #[test]
    fn test_type_layout_budget() {
        assert!(size_of::<LuaTypeDeclId>() <= 8);
        assert!(size_of::<LuaType>() <= 16);
    }
}
