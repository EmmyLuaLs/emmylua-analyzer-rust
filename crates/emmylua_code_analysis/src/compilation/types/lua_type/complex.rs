...
use crate::{AsyncState, InFiled, SemanticModel, first_param_may_not_self};

use super::super::generic_param::GenericParam;
use super::super::type_decl::LuaTypeDeclId;
use super::super::union_type_shallow;
use super::LuaTypeNode;
use super::basic_union::{BasicTypeKind, BasicTypeUnion};
        ...
        for t in &self.types {
            match t {
                LuaType::IntegerConst(i) => {
                    ty = union_type_shallow(ty, LuaType::DocIntegerConst(*i));
                }
                LuaType::FloatConst(_) => {
                    ty = union_type_shallow(ty, LuaType::Number);
                }
                LuaType::StringConst(s) => {
                    ty = union_type_shallow(ty, LuaType::DocStringConst(s.clone()));
                }
                _ => {
                    ty = union_type_shallow(ty, t.clone());
                }
            }
        }
        ...
        if !self.index_access.is_empty() {
            ...
            for (key, value_type) in self.index_access.iter() {
                if matches!(key, LuaType::Integer) {
                    ty = Some(match ty {
                        Some(t) => union_type_shallow(t, value_type.clone()),
